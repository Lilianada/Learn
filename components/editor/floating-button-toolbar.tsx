...
export function FloatingButtonToolbar({ editor, onImageAdd }: FloatingButtonToolbarProps) {
  ...
  return (
    <div className="fixed bottom-4 md:bottom-10 z-50" style={{ 
      right: mainContentWidth > 0 ? 
      ...
      <Button
        ...
        variant="default"
        size="icon"
        className={`h-12 w-12 md:h-14 md:w-14 rounded-full bg-black text-white shadow-lg hover:bg-gray-800 
                   transition-all ${isOpen ? 'rotate-90' : 'rotate-0'}`}
        onClick={() => setIsOpen(!isOpen)}
      >
        {isOpen ? <XCircle className="h-5 w-5 md:h-6 md:w-6" /> : <Keyboard className="h-5 w-5 md:h-6 md:w-6" />}
      </Button>
      ...
      <div 
        ...
        style={{ width: '300px', maxWidth: 'calc(100vw - 40px)', maxHeight: '70vh', overflowY: 'auto' }}
        onClick={handleToolbarClick}
      >
          <div className="flex flex-col space-y-3">
            <h3 className="text-md font-medium border-b pb-2 mb-1">Text Formatting</h3>
            
            {/* Headings */}
            <div className="flex flex-col space-y-2">
              <label className="text-sm font-medium text-muted-foreground">Headings</label>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="sm" className="w-full justify-between">
                    <span>{editor.getAttributes('textStyle').fontSize?.replace('px', '') || '14'} px</span>
                ...
                <DropdownMenuContent align="end">
                  {[12, 14, 16, 18, 20, 24, 30].map((size) => (
                    <DropdownMenuItem
                      key={size}
                      className={editor.getAttributes('textStyle').fontSize === `${size}px` ? "bg-accent" : ""}
                      onClick={() => {
            ...
            {/* Text Styling */}
            <div className="flex flex-col space-y-2">
              <label className="text-sm font-medium text-muted-foreground">Text Style</label>
              <div className="flex gap-1">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className={cn("flex-1", editor.isActive('italic') && "bg-accent")}
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className={cn("flex-1", editor.isActive('underline') && "bg-accent")}
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className={cn("flex-1", editor.isActive('strike') && "bg-accent")}
            ...
            <div className="flex flex-col space-y-2">
              ...
              <div className="flex gap-1">
                <Button
                  variant="outline"
                  size="sm"
                  className={cn("flex-1", editor.isActive({ textAlign: 'left' }) && "bg-accent")}
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className={cn("flex-1", editor.isActive({ textAlign: 'center' }) && "bg-accent")}
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className={cn("flex-1", editor.isActive({ textAlign: 'right' }) && "bg-accent")}
            ...
            <div className="flex flex-col space-y-2">
              ...
              <Popover open={showColors} onOpenChange={setShowColors}>
                ...
                <PopoverContent className="w-64" align="center" onOpenAutoFocus={(e) => e.preventDefault()}>
            ...
            <h3 className="text-md font-medium border-b pb-2 mb-1 mt-2">Lists & Blocks</h3>
            
            {/* Lists */}
            <div className="flex flex-col space-y-2">
              <label className="text-sm font-medium text-muted-foreground">Lists</label>
